--- 
layout: post 
title: Looking Up 
date: 2026-02-12 14:40:00 
description: One third of my photo album is probably made of clouds and skies
tags: nature
---

Whenever I arrive in a new place, I like to record its sky: at different times of the day, under different kinds of weather — although, of course, taking photos of the sky on rainy days can be rather difficult.

Sometimes, I simply stop and look up. The sky feels wide, poetic, and spacious.

<div style="margin: 24px 0;">
  <div style="display: flex; gap: 16px; justify-content: center; align-items: flex-start;">
    <img src="/assets/img/sky1.jpg" alt="Edinburgh" style="width: 32%; height: auto; border-radius: 8px;">
    <img src="/assets/img/sky2.jpg" alt="Geneva" style="width: 32%; height: auto; border-radius: 8px;">
    <img src="/assets/img/sky3.jpg" alt="Nijmegen" style="width: 32%; height: auto; border-radius: 8px;">
  </div>

  <p style="font-size: 0.9em; color: #666; text-align: center; margin-top: 8px;">
    Skys in Edinburgh, Geneva, and Nijmegen.
  </p>
</div>
